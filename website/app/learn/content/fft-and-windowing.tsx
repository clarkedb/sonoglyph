  ...
  return (
    <>
      <p>
        ...
        losslessly and reversibly, as a sum of sine waves. Feed it a block of N samples and it hands
      ...
      <p>
        The <strong>FFT</strong>, the <em>fast</em> Fourier transform, is not a different transform,
        just the trick that makes it affordable: instead of comparing the block against every sine
        wave separately (N² multiplications), it splits the problem in half, then in half again,
        log₂(N) times. The engine that computes every spectrum in this pipeline lives in{' '}
      ...
      <p>
        ...
        Flip the figure to it and watch the leakage rise. <em>Hann</em>, the pipeline’s default, is
        one smooth cosine arch from zero to zero. <em>Hamming</em> is Hann adjusted to hover just
      ...
      <p className="aside">
        ...
        wanted to distinguish Soviet nuclear tests from earthquakes in seismometer data. Only later
        did historians find the same recursion in Gauss’s unpublished notebooks from 1805, invented
        to interpolate asteroid orbits. The FFT was discovered twice, both times to listen to the
        ground and the sky.
      </p>
      <p>
        The tradeoff stops being abstract the moment a real signal shows up. DTMF’s low group sits
        at 697, 770, 852, and 941 Hz, neighbors just 73 Hz apart. At 48 kHz, separating them cleanly
        needs bins meaningfully narrower than that spacing: a 2048-sample FFT (23.4 Hz bins) is the
        floor, and 4096 (11.7 Hz) is comfortable. That is a 43–85 ms analysis window against key
        presses the standard allows to be as short as 40 ms — the recognizer in{' '}
